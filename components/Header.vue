...
<template>
  <header>
    <div
      class="flex justify-between items-center max-w-12xl mx-auto mx-auto sm:px-8 px-4 py-4 sm:py-0"
    >
      <Logo />
      <div class="flex items-center">
        ...
        <nav aria-label="Main Menu" class="opacity-75">
          <ul
            class="flex lg:(space-x-7 mr-15) sm:space-x-2 space-x-1 text-sm md:text-base items-center"
          >
            <template v-for="(link, key) in nav">
              <li
                v-if="link._path !== '/'"
                :key="key"
                :class="key >= 4 ? 'hidden xl:block' : ''"
              >
                <NuxtLink
                  v-slot="slot"
                  :to="link._path"
                  class="!inline-flex items-center group px-2 py-1 lg:px-0"
                ...
                >
                  ...
                  >
                    <icon
                      v-if="link.icon"
                      :name="link.icon"
                      :class="slot?.isActive ? 'opacity-100' : 'opacity-75'"
                      class="hidden text-sm w-3 h-3 lg:inline-block text mr-2 mb-1 transition opacity-75 group-hover:opacity-100"
                    />
                    {{ link.title }}
                  </div>
