...
const component = computed(() => {
  if (isSponsorsOnly.value) return 'div'
...
<template>
  <div class="w-full relative group">
    <TagList
      ...
      :tags="post.tags"
      small
      class="absolute right-0 filter grayscale group-hover:grayscale-0 transition-all"
    />
    <component
      :is="component"
      v-if="!isHidden"
    ...
    >
      <div
        v-if="isSponsorsOnly"
        class="group-hover:flex hidden left-30 w-120 -top-5 page-enter-active absolute rounded-lg px-5 py-3 text-white items-center bg-gray-900/90 z-10"
      >
        <Icon name="material-symbols:lock-outline" class="!w-8 !h-8" />
        <div class="ml-3">
          <p class="mb-1 font-bold">Super secret content</p>
          <p class="text-sm mb-1 opacity-80">
            This post is only available to my
            <a href="https://discord.com/invite/janslounge" target="_blank">
              Discord community
            </a>
            .
          </p>
      ...
      <div
        ...
        :class="[isSponsorsOnly ? 'opacity-50 cursor-not-allowed ' : '']"
      >
        <div
          class="opacity-70 group-hover:(opacity-90) transition-all absolute top-3 -left-20"
        >
          <div class="">
            {{ month }}
          </div>
