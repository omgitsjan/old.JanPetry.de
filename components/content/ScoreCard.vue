...
const percentage = computed(() => {
  ...
  return (((props.after - props.before) / props.before) * 100).toFixed()
})
...
const formattedAfter = computed(() => {
  ...
  return (
    new Intl.NumberFormat('en-AU').format(props.after) + (props.suffix || '')
  )
})
</script>
...
<template>
  <div
    class="p-5 bg-white dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-800 rounded shadow-sm text-gray-900 dark:text-gray-200"
  >
    ...
    <div class="flex items-center pt-1">
      ...
      >
        <svg
          ...
          class="w-4 h-4 mr-1 opacity-70 fill-current"
          viewBox="0 0 24 24"
        >
          <path
            class="heroicon-ui"
